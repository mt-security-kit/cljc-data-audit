...
(defn latin-name-valid?
  ...
  ;   ... it contains latin characters, accented latin characters, digits,
  ;       underscrores, hyphens, apostrophes, periods and/or spaces only.
  ;
  ...
  ([n min max]
   ; TODO
   ; Check whether it contains latin characters.
   ; E.g., '...' should be not a valid latin name.
...
(defn password-valid?
  ...
  ;
  ; @usage
  ; (password-valid? "Password123" 6 16)
  ; =>
  ; true
  ;
  ; @return (boolean)
...
(defn phone-number-valid?
  ...
  ;
  ; @usage
  ; (phone-number-valid? "+123456789" 6 24)
  ; =>
  ; true
  ;
  ; @return (boolean)
...
(defn pin-code-valid?
  ...
  ;
  ; @usage
  ; (pin-code-valid? "420069" 6 8)
  ; =>
  ; true
  ;
  ; @return (boolean)
  ([n]
   (pin-code-valid? n 4 8))

  ([n min]
   (pin-code-valid? n min 8))

  ([n min max]
   (let [pattern (patterns/pin-code-pattern min max)]
        (regex/re-match? (str n) pattern))))
...
(defn username-valid?
  ...
  ;   ... its length is within a certain domain,
  ;   ... it contains latin characters, digits, underscrores and/or hyphens only.
  ;
  ; @param (*) n
  ; @param (integer)(opt) min
  ; Default: 4
